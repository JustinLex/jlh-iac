...
# backup to local
duplicacy -background -log backup -threads 4 -storage local > "/mnt/backup/log/backup-$(date +"%Y-%m-%dZ%T")" 2>&1

# copy to remote
# https://github.com/gilbertchen/duplicacy/wiki/copy
duplicacy -background -log copy -threads 10 -from local -to remote > "/mnt/backup/log/copy-$(date +"%Y-%m-%dZ%T")" 2>&1
